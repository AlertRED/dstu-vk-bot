...
class ScrapePage:
    ...
    def get_schedule(self, group_number, sem=1):

        ScrapePage.headers['referer'] = 'https://edu.donstu.ru/Rasp/Rasp.aspx?group=%s&sem=%s' % (group_number, sem)
        params = (
            ('group', str(group_number)),
            ('sem', '1'),
        )
        response = requests.post('https://edu.donstu.ru/Rasp/Rasp.aspx', headers=ScrapePage.headers, params=params, data=ScrapePage.data)
        ...
        return date.text, group.text, type_schedule.value, self.__foo(tree)

    def __foo(self, tree, index=0, schedule=None):
        if schedule is None:
            schedule = []

        days = tree.xpath('//tr[@id="ctl00_MainContent_ASPxGridView1_DXGroupRowExp' + str(index) + '"]')
        if len(days):
            day_name = days[0].xpath('td[@class="dxgv"]')[1].text
            schedule.append({'day_name': day_name, "schedule": []})
        else:
            subject = tree.xpath('//tr[@id="ctl00_MainContent_ASPxGridView1_DXDataRow' + str(index) + '"]')
            if subject:
                schedule[-1]['schedule'].append(tuple(map(lambda x: x.text, subject[0].xpath('td')[1::])))
            else:
                return schedule
        return self.__foo(tree, index + 1, schedule)